...
def xtime(a):
    if (a & 0b10000000 ):
        return (((a << 1) ^ 0b00011011) & 0b11111111)
    else:
        return (a << 1)
...
def mix_single_column(a):
    ...
    a[0] = xtime(b[0]) ^ b[1] ^ b[2] ^ b[3]
    a[1] = b[0] ^ b[1]  ^ b[2] ^ xtime(b[2]) ^ xtime(b[3])
    a[2] = b[0] ^ xtime(b[1]) ^ b[2] ^ b[3] ^ xtime(b[3])
    a[3] = b[0] ^ b[1] ^ xtime(b[1]) ^ xtime(b[2]) ^ b[3]
...
def encrypt(key, plaintext, workload=100000):
    ...
    salt = random.randbytes(SALT_SIZE)
